...
from python_hw_number_9_part2.date.users import User
...
class Registration_page:
    ...
    def register(self, user: User):
        ...
        self.select_hobbies()
        self.resource_path(user.picture)
    ...
    def should_have_registered(self, user: User):
        ...
        browser.all('table tbody td').should(have.exact_texts(
            'Student Name', f"{user.first_name} {user.last_name}",
                    'Student Email', user.email,
                    'Gender',user.gender,
                    'Mobile',user.mobile_number,
                    'Date of Birth',f"{user.date_of_bith['day']} {user.date_of_bith['month']},{user.date_of_bith['year']}",
